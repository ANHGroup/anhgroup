<div class="container">
  <div class="row">
    <div class="blog_section">
      <div class="single_blog col-md-4 slider-content">
          <a href="https://www.anhenterprise.com/">
            <img src="<?php echo e(asset('public/img/companies/ANH_Enterprise_Limited_Logo.png')); ?>" class="" alt="ANH Enterprise">
          
            <div class="slider-text">
              <h3>ANH Enterprise</h3>
              <p>ANH Enterprise Limited is one of the leading Warehouse Solution, Material Handling, Fabric Makeup, Dyeing & Automation Solution Companies in Bangladesh.</p>
            </div>
          </a>
        </div>
      <div class="single_blog col-md-4 slider-content">
          <a href="https://www.srdlbd.com/">
            <img src="<?php echo e(asset('public/img/companies/ANH_Limited_Logo.png')); ?>" class="" alt="ANH Limited">
            <div class="slider-text">
              <h3>ANH Limited</h3>
              <p>ANH Limited is one of the leading Warehouse Solution, Material Handling, Fabric Makeup, Dyeing & Automation Solution Companies in Bangladesh.</p>
            </div>
          </a>
        </div>
      <div class="single_blog col-md-4 slider-content">
        <a href="https://www.srdlbd.com/">
          <img src="<?php echo e(asset('public/img/companies/SRD_Limited_Logo.png')); ?>" class="" alt="SRD Limited">
          <div class="slider-text">
            <h3>SRD Limited</h3>
            <p>SRD Limited is one of the top construction company in Bangladesh, and we are working together to ensure your construction and business needs are met.</p>
          </div>
        </a>
      </div>
      <div class="single_blog col-md-4 slider-content">
        <a href="https://www.raylifebd.com/">
          <img src="<?php echo e(asset('public/img/companies/RAY_Life_Logo.png')); ?>" class="" alt="RAY Life BD">
          <div class="slider-text">
            <h3>RAY Life BD</h3>
            <p>Ray is Smartly Moving Lives with range of life saving products and services. This brand is committed to ensure the utmost quality. Our ultimate objective is to bring happy shine on faces.</p>
          </div>
        </a>
      </div>
      <div class="single_blog col-md-4 slider-content">
        <a href="https://www.raydeostation.com/">
          <img src="<?php echo e(asset('public/img/companies/RAYDEO.png')); ?>" class="" alt="RAYDEO">
          <div class="slider-text">
            <h3>RAYDEO</h3>
            <p>Listen to RAY DEO – when you are in a bad mood and you want to change your mood from bad to good.</p>
          </div>
        </a>
      </div>
    </div>
  </div>
</div><?php /**PATH E:\1_ANH Group\Website\anhgroup\resources\views/layout/manufacturer.blade.php ENDPATH**/ ?>